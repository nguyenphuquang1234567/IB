...
import { BA_STEP_IDS } from "@/lib/business-acumen/learning-path";

export type BAMasteryLevel = "not-started" | "learning" | "practicing" | "mastered";

export interface BAStepProgress {
  stepId: string;
  ...
  masteryLevel: BAMasteryLevel;
}

export interface BAState {
  stepProgress: Record<string, BAStepProgress>;
  streakDays: number;
  ...
  setMasteryLevel: (stepId: string, level: BAMasteryLevel) => void;
  recordVisit: (stepId: string) => void;
  getStepProgress: (stepId: string) => BAStepProgress;
...
function getInitialProgress(stepId: string): BAStepProgress {
  return {
    stepId,
    lessonsViewed: [],
...
export const useBusinessAcumenStore = create<BAState>()(
  persist(
    (set, get) => ({
      stepProgress: {},
      streakDays: 0,
      ...
      markLessonViewed: (stepId, lessonId) => {
        set((state) => {
          ...
          const masteryLevel: BAMasteryLevel =
      ...
      getOverallProgress: () => {
        ...
        let total = 0;
        let completed = 0;
        for (const stepId of BA_STEP_IDS) {
          const step = state.stepProgress[stepId] ?? getInitialProgress(stepId);
          total += 3;
