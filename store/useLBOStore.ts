...
import { LBO_STEP_IDS } from "@/lib/lbo-lab/learning-path";

export type LBOMasteryLevel = "not-started" | "learning" | "practicing" | "mastered";

export interface LBOStepProgress {
  stepId: string;
  ...
  masteryLevel: LBOMasteryLevel;
}

export interface LBOState {
  stepProgress: Record<string, LBOStepProgress>;
  streakDays: number;
  ...
  setMasteryLevel: (stepId: string, level: LBOMasteryLevel) => void;
  recordVisit: (stepId: string) => void;
  getStepProgress: (stepId: string) => LBOStepProgress;
...
function getInitialProgress(stepId: string): LBOStepProgress {
  return {
    stepId,
    lessonsViewed: [],
...
export const useLBOStore = create<LBOState>()(
  persist(
    (set, get) => ({
      stepProgress: {},
      streakDays: 0,
      ...
      markLessonViewed: (stepId, lessonId) => {
        set((state) => {
          ...
          const masteryLevel: LBOMasteryLevel =
      ...
      getOverallProgress: () => {
        ...
        let total = 0;
        let completed = 0;
        for (const stepId of LBO_STEP_IDS) {
          const step = state.stepProgress[stepId] ?? getInitialProgress(stepId);
          total += 3;
