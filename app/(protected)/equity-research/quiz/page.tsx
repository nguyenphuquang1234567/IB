...
import { equityResearchQuestions } from "@/lib/questions/equity-research";
...
function getRankLabel(accuracy: number) {
  if (accuracy >= 85) return "Analyst Ready";
  if (accuracy >= 75) return "On Track";
  if (accuracy >= 60) return "Keep Practicing";
  return "Review Fundamentals";
}

export default function EquityResearchQuizPage() {
  const router = useRouter();
  const [questions, setQuestions] = useState<Question[]>(equityResearchQuestions);
  ...
  const handleReset = useCallback(() => {
    setQuestions(shuffleArray(equityResearchQuestions));
    setCurrentIndex(0);
    setAnswers({});
    setRevealed({});
    setFinished(false);
