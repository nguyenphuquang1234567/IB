...
import { PRACTICE_QUESTIONS } from "@/lib/budgeting/practice";
...
function isCorrect(userAnswer: number, correctAnswer: number, unit: string): boolean {
  const tolerance = unit === "$" ? Math.max(1, correctAnswer * 0.01) : 0.5;
  return Math.abs(userAnswer - correctAnswer) <= tolerance;
}

export default function BudgetingPracticePage() {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [answers, setAnswers] = useState<Record<string, number>>({});
  const [revealed, setRevealed] = useState<Record<string, boolean>>({});
  const [finished, setFinished] = useState(false);

  const q = PRACTICE_QUESTIONS[currentIndex];
  const userAnswer = q ? answers[q.id] : undefined;
  const isRevealed = q ? !!revealed[q.id] : false;
  const correct = q && userAnswer !== undefined && isRevealed && isCorrect(userAnswer, q.answer, q.unit);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Test your budgeting and forecasting knowledge
        </p>
      </div>
                ...
                {isRevealed && (
                  <div className="mt-6 space-y-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn("font-medium", correct ? "text-emerald-600" : "text-red-600")}>
                        {correct
                          ? "Correct!"
                          : `Correct answer: ${formatAnswer(q!.answer, q!.unit)}`}
                      </span>
                    </div>
                    <div className="rounded-lg bg-muted/50 p-3 text-sm">
                      <p className="font-mono text-xs mb-1">{q?.formula}</p>
                      <p className="text-muted-foreground">{q?.explanation}</p>
                    </div>
                    <Button onClick={handleNext} className="w-full">
                      {currentIndex < PRACTICE_QUESTIONS.length - 1 ? (
                        <>
