...
import { computeLBO, fmt } from "@/lib/lbo-lab/engine";
...
export default function LBOLabPage() {
  ...
  const currentModule = LBO_LEARNING_MODULES[learnModuleIndex] as LearningModule;
  ...
  const handleModuleComplete = useCallback(() => {
    ...
    if (learnModuleIndex < LBO_LEARNING_MODULES.length - 1) {
  ...
  const sensitivityData = useMemo(() => {
    ...
    return multiples.map((m) => {
      ...
      const exitEquity = exitEv - assumptions.debtAmount;
      const moic = assumptions.equityAmount > 0 ? exitEquity / assumptions.equityAmount : 0;
      const irr = assumptions.equityAmount > 0 && exitEquity > 0
    ...
    <div className="max-w-[1600px] mx-auto space-y-4">
      ...
      >
        <div>
          ...
          <p className="text-sm text-finstep-brown/60 dark:text-muted-foreground font-nunito mt-1">
            Master leveraged buyouts: structure, sources & uses, MOIC, and IRR
          </p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        <TabsList className="grid w-full grid-cols-3 h-11 bg-finstep-beige/50 dark:bg-muted/30">
          <TabsTrigger value="learn" className="data-[state=active]:bg-emerald-500 data-[state=active]:text-white gap-1.5 text-xs font-bold">
            <BookOpen className="w-4 h-4" />
            <span className="hidden sm:inline">Learning</span>
          </TabsTrigger>
          <TabsTrigger value="build" className="data-[state=active]:bg-emerald-500 data-[state=active]:text-white gap-1.5 text-xs font-bold">
            <Hammer className="w-4 h-4" />
            <span className="hidden sm:inline">Build</span>
          </TabsTrigger>
          <TabsTrigger value="sensitivity" className="data-[state=active]:bg-emerald-500 data-[state=active]:text-white gap-1.5 text-xs font-bold">
              ...
              <Card className="shadow-sm border-border/40">
                <CardHeader className="pb-2 pt-4 px-4">
                  <CardTitle className="text-xs font-bold uppercase tracking-wider text-finstep-brown/60 dark:text-muted-foreground">
                    Modules ({completedModules.size}/{LBO_LEARNING_MODULES.length})
                  </CardTitle>
                </CardHeader>
                <CardContent className="px-2 pb-3">
                  <div className="space-y-1">
                    {LBO_LEARNING_MODULES.map((mod, i) => {
              ...
              <Card className="shadow-sm border-border/40">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        disabled={learnModuleIndex === LBO_LEARNING_MODULES.length - 1}
                        className="h-8 px-2"
                      >
                ...
                <CardContent className="pt-5 space-y-5">
                  <div className="p-4 rounded-xl bg-emerald-500/5 dark:bg-emerald-500/10 border border-emerald-500/20">
                    <p className="text-base font-mono font-bold text-emerald-600 dark:text-emerald-400 whitespace-pre-line">
                    ...
                    <div className="space-y-3">
                      ...
                      {!quizSubmitted ? (
                        <Button
                          ...
                          className="h-9 text-sm bg-emerald-500 text-white hover:bg-emerald-600"
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <Button
                            ...
                            className="h-9 text-sm bg-emerald-500 text-white hover:bg-emerald-600"
                          >
                            {learnModuleIndex < LBO_LEARNING_MODULES.length - 1 ? (
                              <>
                                Next Module
                ...
                <CardContent>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <div className="rounded-lg border border-border/40 p-3">
                      <p className="text-[10px] text-finstep-brown/60 dark:text-muted-foreground">Entry Multiple</p>
                      <p className="text-lg font-bold tabular-nums text-finstep-brown dark:text-foreground">{output.entryMultiple.toFixed(1)}x</p>
                    </div>
                    <div className="rounded-lg border border-border/40 p-3">
                      <p className="text-[10px] text-finstep-brown/60 dark:text-muted-foreground">Exit EV</p>
                      <p className="text-lg font-bold tabular-nums text-finstep-brown dark:text-foreground">{fmt(output.exitEv)}</p>
                    </div>
                    <div className="rounded-lg border border-border/40 p-3">
                      <p className="text-[10px] text-finstep-brown/60 dark:text-muted-foreground">MOIC</p>
                      <p className="text-lg font-bold tabular-nums text-finstep-brown dark:text-foreground">{output.moic.toFixed(2)}x</p>
                    </div>
                  ...
                  <p className="text-[10px] text-finstep-brown/60 dark:text-muted-foreground mt-3">
                    MOIC = Exit Equity / Initial Equity. IRR = annualized return over hold period.
                  </p>
                </CardContent>
              </Card>
            </div>
          ...
          <Card className="shadow-sm border-border/40">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm flex items-center gap-2 text-finstep-brown dark:text-foreground">
                ...
                Exit Multiple Sensitivity (MOIC & IRR)
              </CardTitle>
            </CardHeader>
            <CardContent>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-border/40">
                      <th className="py-2 px-3 text-finstep-brown/60 dark:text-muted-foreground font-medium text-left">Exit EV/EBITDA</th>
                      <th className="py-2 px-3 text-finstep-brown/60 dark:text-muted-foreground font-medium text-right">Exit EV</th>
                      <th className="py-2 px-3 text-finstep-brown/60 dark:text-muted-foreground font-medium text-right">MOIC</th>
                      <th className="py-2 px-3 text-finstep-brown/60 dark:text-muted-foreground font-medium text-right">IRR</th>
                    </tr>
                  </thead>
                  <tbody>
                    {sensitivityData.map((row) => (
                      <tr
                        ...
                        className={cn(
                          ...
                          row.multiple === assumptions.exitMultiple && "bg-emerald-500/10 dark:bg-emerald-500/20"
                        )}
                      >
              ...
              <p className="text-[10px] text-finstep-brown/60 dark:text-muted-foreground mt-3">
                Exit EBITDA: {fmt(assumptions.exitEbitda)}. Each 1x change in exit multiple significantly impacts MOIC and IRR.
              </p>
            </CardContent>
          </Card>
        </TabsContent>
