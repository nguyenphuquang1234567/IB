...
import { INVESTMENT_PRACTICE } from "@/lib/corporate-finance/practice";
...
function isCorrect(userAnswer: number, correctAnswer: number, unit: string): boolean {
  const tolerance = unit === "$" ? 5 : unit === "yrs" ? 0.1 : 0.5;
  return Math.abs(userAnswer - correctAnswer) <= tolerance;
}

export default function InvestmentPracticePage() {
  ...
  const q = INVESTMENT_PRACTICE[currentIndex];
  ...
  const stats = useMemo(() => {
    ...
    const correctCount = INVESTMENT_PRACTICE.filter(
      (x) => answers[x.id] !== undefined && isCorrect(answers[x.id], x.answer, x.unit)
    ).length;
    return {
      answered,
      correct: correctCount,
      total: INVESTMENT_PRACTICE.length,
  ...
  const handleNext = useCallback(() => {
    if (currentIndex < INVESTMENT_PRACTICE.length - 1) {
  ...
  useEffect(() => {
    if (finished && stats.answered > 0) {
      completePractice("investment", stats.accuracy);
    }
  }, [finished, stats.answered, stats.accuracy, completePractice]);

  const formatAnswer = (n: number, unit: string) => {
    if (unit === "$") return `$${n}`;
    if (unit === "%") return `${n}%`;
    if (unit === "yrs") return `${n} years`;
    return String(n);
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Test your NPV, IRR, and capital budgeting knowledge
        </p>
      </div>
            ...
            <Card>
              <CardContent className="pt-6 pb-6">
                <p className="text-xs text-muted-foreground mb-2">
                  Question {currentIndex + 1} of {INVESTMENT_PRACTICE.length}
                </p>
                  ...
                  <div className="mt-4 space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn("text-sm font-medium", correct ? "text-emerald-600" : "text-red-600")}>
                        {correct ? "Correct!" : `Answer: ${formatAnswer(q?.answer ?? 0, q?.unit ?? "")}`}
                      </span>
                    </div>
