...
import { INVESTMENT_CONCEPTS } from "@/lib/corporate-finance/investment";
...
export default function InvestmentLearningPage() {
  ...
  const [expandedId, setExpandedId] = useState<string | null>("npv");

  const handleExpand = (conceptId: string) => {
    const next = expandedId === conceptId ? null : conceptId;
    setExpandedId(next);
    if (next) markLessonViewed("investment", next);
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          NPV, IRR, payback period, and capital budgeting
        </p>
      </div>

      <div className="space-y-4">
        {INVESTMENT_CONCEPTS.map((concept) => (
