...
import { CASH_FLOW_CONCEPTS } from "@/lib/corporate-finance/cash-flow";
...
export default function CashFlowLearningPage() {
  ...
  const [expandedId, setExpandedId] = useState<string | null>("ocf");

  const handleExpand = (conceptId: string) => {
    const next = expandedId === conceptId ? null : conceptId;
    setExpandedId(next);
    if (next) markLessonViewed("cash-flow", next);
  };
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-varela font-bold text-finstep-brown dark:text-foreground">
          Cash Flow & Liquidity Fundamentals
        </h1>
        <p className="text-sm text-muted-foreground mt-1">
          Operating cash flow, working capital, CCC, and liquidity risk
        </p>
      </div>

      <div className="space-y-4">
        {CASH_FLOW_CONCEPTS.map((concept) => (
