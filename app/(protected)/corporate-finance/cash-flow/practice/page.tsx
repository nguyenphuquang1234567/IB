...
import { CASH_FLOW_PRACTICE } from "@/lib/corporate-finance/practice";
...
function isCorrect(userAnswer: number, correctAnswer: number, unit: string): boolean {
  const tolerance = unit === "$" ? 5 : unit === "days" ? 2 : 0.05;
  return Math.abs(userAnswer - correctAnswer) <= tolerance;
}

export default function CashFlowPracticePage() {
  ...
  const q = CASH_FLOW_PRACTICE[currentIndex];
  ...
  const stats = useMemo(() => {
    ...
    const correctCount = CASH_FLOW_PRACTICE.filter(
      (x) => answers[x.id] !== undefined && isCorrect(answers[x.id], x.answer, x.unit)
    ).length;
    return {
      answered,
      correct: correctCount,
      total: CASH_FLOW_PRACTICE.length,
  ...
  const handleNext = useCallback(() => {
    if (currentIndex < CASH_FLOW_PRACTICE.length - 1) {
  ...
  useEffect(() => {
    if (finished && stats.answered > 0) {
      completePractice("cash-flow", stats.accuracy);
    }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Test your cash flow and liquidity knowledge
        </p>
      </div>
            ...
            <Card>
              <CardContent className="pt-6 pb-6">
                <p className="text-xs text-muted-foreground mb-2">
                  Question {currentIndex + 1} of {CASH_FLOW_PRACTICE.length}
                </p>
                ...
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  />
                  {q?.unit === "days" && <span className="text-muted-foreground">days</span>}
                  ...
                  <div className="mt-4 space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn("text-sm font-medium", correct ? "text-emerald-600" : "text-red-600")}>
                        {correct ? "Correct!" : `Answer: ${q?.unit === "days" ? q?.answer + " days" : q?.unit === "x" ? q?.answer + "x" : "$" + q?.answer}`}
                      </span>
                    </div>
