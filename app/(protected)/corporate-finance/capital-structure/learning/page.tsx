...
import { CAPITAL_STRUCTURE_CONCEPTS } from "@/lib/corporate-finance/capital-structure";
...
export default function CapitalStructureLearningPage() {
  ...
  const [expandedId, setExpandedId] = useState<string | null>("cost-of-debt");

  const handleExpand = (conceptId: string) => {
    const next = expandedId === conceptId ? null : conceptId;
    setExpandedId(next);
    if (next) markLessonViewed("capital-structure", next);
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Cost of debt, cost of equity, WACC, optimal structure, and financial leverage
        </p>
      </div>

      <div className="space-y-4">
        {CAPITAL_STRUCTURE_CONCEPTS.map((concept) => (
