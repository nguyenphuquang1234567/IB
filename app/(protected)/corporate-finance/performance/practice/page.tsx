...
import { PERFORMANCE_PRACTICE } from "@/lib/corporate-finance/practice";
...
export default function PerformancePracticePage() {
  ...
  const q = PERFORMANCE_PRACTICE[currentIndex];
  ...
  const stats = useMemo(() => {
    ...
    const correctCount = PERFORMANCE_PRACTICE.filter(
      (x) => answers[x.id] !== undefined && isCorrect(answers[x.id], x.answer, x.unit)
    ).length;
    return {
      answered,
      correct: correctCount,
      total: PERFORMANCE_PRACTICE.length,
  ...
  const handleNext = useCallback(() => {
    if (currentIndex < PERFORMANCE_PRACTICE.length - 1) {
  ...
  useEffect(() => {
    if (finished && stats.answered > 0) {
      completePractice("performance", stats.accuracy);
    }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Test your financial performance analysis knowledge
        </p>
      </div>
            ...
            <Card>
              <CardContent className="pt-6 pb-6">
                <p className="text-xs text-muted-foreground mb-2">
                  Question {currentIndex + 1} of {PERFORMANCE_PRACTICE.length}
                </p>
